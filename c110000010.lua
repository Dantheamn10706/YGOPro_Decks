...
function s.egpcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_GRAVE) or 
		   e:GetHandler():IsPreviousLocation(LOCATION_HAND) or
		   e:GetHandler():IsPreviousLocation(LOCATION_DECK) or
		   e:GetHandler():IsPreviousLocation(LOCATION_REMOVED)
end
...
function s.valcheck(e,c)
	...
	--if Duel.IsExistingMatchingCard(aux.FilterFaceupFunction(Card.IsCode,21208154),c:GetControler(),LOCATION_MZONE,0,1,nil) then
